...
function showTip(e,name){
  ...
  const forgeLevel=name.match(/\+(\d+)$/)?parseInt(name.match(/\+(\d+)$/)[1]):0;
  const it=ITEMS[baseName];if(!it)return;
  ...
  const baseAtk=it.atk||0, baseDef=it.def||0;
  const bonus=forgeBonus(name);
  const statStr=it.atk?`⚔️ 공격력 +${baseAtk+bonus}${forgeLevel>0?` (기본${baseAtk}+강화${bonus})`:''}`:
                it.def?`🛡️ 방어력 +${baseDef+bonus}${forgeLevel>0?` (기본${baseDef}+강화${bonus})`:''}`:
                it.hp?`💊 HP +${it.hp>=9999?'전체':it.hp}`:'';
  document.getElementById('ts').textContent=statStr;
  document.getElementById('td').textContent=it.desc+(forgeLevel>0?` (강화 +${forgeLevel})`:'');
  tt.style.display='block';
  // 오른쪽 잘림 방지
  const ttW=200;
  const x=e.clientX+12;
  tt.style.left=(x+ttW>window.innerWidth ? e.clientX-ttW-8 : x)+'px';
